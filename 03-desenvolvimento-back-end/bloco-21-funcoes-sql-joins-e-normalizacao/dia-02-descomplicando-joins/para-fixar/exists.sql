...
-- Criando db Hotel que será usado
...
SELECT b.Id, b.title FROM hotel.Books AS b
WHERE EXISTS (
	SELECT returned FROM hotel.Books_Lent AS bl
    WHERE bl.returned = 0 AND b.title LIKE '%lost%'
);
...
SELECT c.`Name`, cr.`Name`  FROM hotel.Customers AS c
JOIN hotel.CarSales AS cs ON c.CustomerID = cs.CustomerID
JOIN hotel.Cars AS cr ON cs.CarID = cr.Id
WHERE EXISTS (
	SELECT * FROM hotel.CarSales AS cs
    WHERE c.CustomerID = cs.CustomerID
);