...
export class LensController extends Controller<ILens> {
  private _route: string;

  constructor(service = new LensService(), route = "/lenses") {
    super(service);
    this._route = route;
  }
  ...
  create = async (
    req: RequestWithBody<ILens>,
    res: Response<ILens | ResponseError>
  ): Promise<typeof res> => {
    const { body } = req;
    try {
