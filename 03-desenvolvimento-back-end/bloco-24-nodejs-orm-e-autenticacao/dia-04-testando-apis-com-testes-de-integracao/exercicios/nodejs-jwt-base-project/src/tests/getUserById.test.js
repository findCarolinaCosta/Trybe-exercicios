...
describe("Rota GET /api/users/:id", () => {
  describe("Sucesso em fazer requisição com token", () => {
    ...
    it("Efetua login corretamente e retorna token", async () => {
      response = await chai.request(server).post("/api/login").send({
    ...
    it("com token válido usuário poderá ver somente os seus próprios dados", async () => {
      const { token } = response.body;
  ...
  describe("Falha ao fazer requisição sem token", () => {
    it(
      'Esperado status 400 e message "Token não ' +
        'encontrado ou informado" ao fazer requisição sem token',
      async () => {
        const userResponse = await chai.request(server).get("/api/users/1");

        expect(userResponse).to.have.status(400);
        expect(userResponse.body.message).to.be.equal(
